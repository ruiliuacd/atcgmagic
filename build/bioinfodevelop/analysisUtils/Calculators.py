...
import numpy as np
from bioinfodevelop.analysisUtils import variantUtils,Utils

seqerrorrate=0.008
outgroupidx_in_topleveltable=[8,10]#outgidx=7;outg2idx=9
class Caculator():
    def __init__(self):
        ...
        for vcfconfigf in [tvcfconfig[0],rvcfconfig[0]]:
            ...
            for line in vcfconfig:
                ...
                if vcffilename_obj!=None:
                    ...
                    self.vcfnameKEY_vcfobj_pyBAMfilesVALUE[vcfname].append(variantUtils.VCF_Data(vcfname))
...
class Caculate_S_ObsExp_difference(Caculator):
        ...
        for vcfconfigfilename in listOftargetpopvcfconfig[:]+listOfrefpopvcffileconfig[:]:
            ...
            for line in vcfconfig:
                ...
                if vcffilename_obj!=None:
                    ...
                    self.vcfnameKEY_vcfobj_pyBAMfilesVALUE[vcfname].append(pysam.AlignmentFile(line.strip(),'rb'))
            vcfconfig.close()
            if re.search(r"indvd[^/]+",vcfname)!=None:
                self.MethodToSeqpoplist.append("indvd")
    ...
    def process(self,T):
        ...
        if len(T[1]) != len(T[2]) or len(T[2])!=1  or len(T[2])!=1:
            return
        snprec_in_toplevel=self.dbvariantstoolstojudgeancestral.operateDB("select","select * from "+self.topleveltablejudgeancestralname+" where chrID='"+self.currentchrID+"' and snp_pos='"+str(T[0])+"'")
        #new version instead of """ code block """
        OUTgroups=Utils.getOUTgrouptypeForOneSNP(snprec_in_toplevel,outgroupidx_in_topleveltable)
        Anc_base_idx=Utils.judgeAncestralalle1(OUTgroups,'ALT',self.mindepthtojudefixed)
        if Anc_base_idx[0] :
